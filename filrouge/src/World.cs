...
namespace FilRouge
{
	public class World
	{
		...
		public World()
		{
			...
			floor = new List<Room> { };
			peuple = new List<Character> { };
			index = new Dictionary<RGBCode, string>();
		}
		...
		public World(string name)
		{
			...
			floor = new List<Room> { };
			peuple = new List<Character> { };
			index = readLibrary(name);
		}

		//permet de lire un index d'objets dans un fichier txt
		public Dictionary<RGBCode, string> readLibrary(string name)
		{
			Dictionary<RGBCode, string> code = new Dictionary<RGBCode, string>();
			RGBCode transition;
			string nom;
			// Example #2
		...
		public World(int size, List<Room> list)
		{
			clear = false;
			nbSalles = size;
				...
				for (int j = 0; j < sizeH; j++)
				{
					...
					if (index.ContainsKey(id) == true)
					{
						if (index[id] == "salle beacon")
						{
							...
							floor.Add(new Room(nbSalles, new Point(xB, yB), test));
				...
				for (int j = 0; j < sizeH; j++)
				{
					...
					if (index.ContainsKey(id) == true)
					{
						if (index[id] == "salle beacon")
						{
							...
							Point test = new Point(longueur, largeur);
							floor.Add(new Room(nbSalles, new Point(xB, yB), test, compteurportes));
							//Console.WriteLine("ajout de la salle n° " + nbSalles);
							nbSalles++;
							j = xB;
							i = yB;
			...
			foreach (Room room in floor)
			{
				...
				for (int i = 1; i < height; i++)
				{
					for (int j = 1; j < width; j++)
					{
						...
						G = clr.G;
						B = clr.B;
						RGBCode idObject = new RGBCode(R, G, B);
						...
						if (index.ContainsKey(idObject) && index[idObject] != "vide de gimp")
						{
							...
							nom = info[0];
							entity_type = int.Parse(info[1]);
							id = short.Parse(info[2]);
							shape_type = int.Parse(info[3]);
							int string_index = 3;
							rec = new Rectangle();
							switch (shape_type)
							{
								case 0:
									...
									rec.SetLongueurRectangle(int.Parse(info[++string_index]));
									rec.SetLargeurRectangle(int.Parse(info[++string_index]));

									break;
							}
							if (int.Parse(info[++string_index]) == 0)
							{
								passThrough = false;
							}
							else
		...
		{
			foreach (KeyValuePair<RGBCode, string> line in index)
			{
		...
		public Dictionary<RGBCode, string> GetIndex()
		{
			return index;
		}
		...
		public bool VerifIntersectionSalles()
		{
			...
			foreach (Room room in floor)
			{
				foreach (Room roomtest in floor)
				{
		...
		{
			foreach (Character johnDoe in peuple)
			{
		...
		public List<Character> GetPeuple()
		{
			return peuple;
		}
		...
		public Room GetSallePersonnage(Character johnDoe, int idsalle)
		{
			foreach (Room room in floor)
			{
		...
		List<Room> floor;
		bool clear;
		Dictionary<RGBCode, string> index;
		List<Character> peuple;
