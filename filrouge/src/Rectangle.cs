...
namespace FilRouge
{
	public class Rectangle : Shape//hérite de la classe forme
	{
		...
		{
			return new Point(centre.getAbscisse() - longueur / 2, centre.getOrdonnee() - largeur / 2);
		}
		...
		public bool AppartenanceRectangle2(Rectangle a)
		{
			...
			appart = AppartenanceRectangle(supgauche) || AppartenanceRectangle(infgauche) || AppartenanceRectangle(supdroit) || AppartenanceRectangle(infdroit);
		...
		{
			Console.WriteLine("C'est un rectangle de centre X : " + centre.getAbscisse() + " Y :" + centre.getOrdonnee());
